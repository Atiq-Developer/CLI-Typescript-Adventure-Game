...
while (player.Power > 0 && opponent.Power > 0) {
    ...
    if (action == "Attack") {
        ...
        if (outcome > 0.5) {
            ...
            console.log(chalk.bold.red(`${opponent.name}'s power is ${opponent.Power}`));
        }
        else {
        ...
        console.log(chalk.bold.green(`${player.name}'s power is restored to ${player.Power}`));
    }
    ...
    if (opponent.Power <= 0) {
        console.log(chalk.green.bold.italic(`Congratulations, ${player.name}! You defeated the ${opponent.name}.`));
        break;
    }
    else if (player.Power <= 0) {
        console.log(chalk.red.bold.italic("You were defeated... Better luck next time!"));
        break;
    }
}
